...
export default function LaborModel() {
  ...
  const [laborItems, setLaborItems] = useState<LaborItem[]>(() => data?.labor || []);
  ...
  const getTotalMonthlyCost = (): number => {
    return laborItems.reduce((total, item) => total + calculateMonthlyWage(item), 0);
  };

  const getDirectLaborCost = (): number => {
    return laborItems
      .filter(item => item.type === 'direct')
      .reduce((total, item) => total + calculateMonthlyWage(item), 0);
  };

  const getIndirectLaborCost = (): number => {
    return laborItems
      .filter(item => item.type === 'indirect')
      .reduce((total, item) => total + calculateMonthlyWage(item), 0);
  };
  ...
  const handleSaveLaborItem = () => {
    ...
    const updatedLaborItems = editingIndex !== null
      ? laborItems.map((item, index) => index === editingIndex ? itemToSave : item)
      : [...laborItems, itemToSave];

    setLaborItems(updatedLaborItems);
    financeEngine.updateLabor(updatedLaborItems);
    onUpdate();
  ...
  const handleDelete = (index: number) => {
    const updatedLaborItems = laborItems.filter((_, i) => i !== index);
    setLaborItems(updatedLaborItems);
    financeEngine.updateLabor(updatedLaborItems);
    onUpdate();
  };
  ...
  const directLaborItems = laborItems.filter(item => item.type === 'direct');
  const indirectLaborItems = laborItems.filter(item => item.type === 'indirect');
