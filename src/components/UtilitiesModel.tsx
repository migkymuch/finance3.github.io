...
export default function UtilitiesModel() {
  ...
  const [utilities, setUtilities] = useState<UtilityItem[]>(() => data?.utilities || []);
  ...
  const handleSaveUtility = () => {
    ...
    financeEngine.updateUtilities(updatedUtilities);
    onUpdate();
  ...
  const electricUtilities = utilities.filter(u => u.type === 'electric');
  const lpgUtilities = utilities.filter(u => u.type === 'lpg');
  const waterUtilities = utilities.filter(u => u.type === 'water');
