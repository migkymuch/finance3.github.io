  ...
  private loadFromStorage(): void {
    ...
    if (storedScenarios) {
      try {
        ...
        console.error('Failed to load stored scenarios:', e);
      }
    }
  }
  ...
  importJSON(jsonString: string): { success: boolean; error?: string } {
    try {
      ...
      try {
        ...
        this.data = backup.data;
        this.scenarios = backup.scenarios;
        return { success: false, error: `Data validation failed: ${validationError}` };
      }
    } catch (e) {
      console.error('Failed to import JSON:', e);
      return { success: false, error: `JSON parsing failed: ${e.message}` };
    }
  }
